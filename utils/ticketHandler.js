...
function buildTicketButtons(claimedBy = null) {
  ...
  const row2 = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId('ticket_refresh').setLabel('Refresh').setEmoji({ id: '1506186758167990334' }).setStyle(ButtonStyle.Secondary),
  );
  return [row1, row2];
}
...
async function handleButton(interaction, client) {
  ...
  if (interaction.customId === 'ticket_refresh') {
    ...
    if (!hasAdmin && !hasRole && !isOwner) {
      return interaction.reply({ content: '❌ بس الإداريين يقدرون يستخدمون Refresh!', ephemeral: true });
    }

    await interaction.message.edit({ components: buildTicketButtons(ticket.claimedBy) }).catch(() => {});
    await interaction.reply({ content: '🔄 تم تحديث الأزرار!', ephemeral: true });
    return;
  }
